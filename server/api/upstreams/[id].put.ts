...
export default defineEventHandler(async (event) => {
  ...
  const { name, baseUrl, apiKeys, aimodels, remark, sortOrder, disabled, proxyId, upstreamPlatform, showUserBalance } = body
  ...
  if (showUserBalance !== undefined) {
    updateData.showUserBalance = showUserBalance
  }
  ...
  if (aimodels !== undefined) {
    ...
    await aimodelService.syncByUpstream(
      ...
      (aimodels as AimodelInput[]).map(m => ({
        ...
        vendor: m.vendor,  // 厂商
        uiCapabilities: m.uiCapabilities,  // UI 能力配置

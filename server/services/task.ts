...
import { tasks, upstreams, aimodels, users, type Task, type TaskStatus, type TaskType, type Upstream, type Aimodel, type ModelType, type ApiFormat } from '../database/schema'
...
import { downloadFile, getFileUrl, readFileAsBase64, saveBase64File, saveFileUnified, getStorageType } from './file'
...
export function useTaskService() {
  ...
  // 获取任务及其上游配置和模型（内部使用）
  async function getTaskWithConfig(id: number): Promise<{ task: Task; upstream: Upstream; aimodel: Aimodel; userApiKey: string | null } | undefined> {
    ...
    return { task, upstream, aimodel, userApiKey: user?.apiKey || null }
  }
  ...
  async function getTaskWithSummary(id: number): Promise<{ task: Task; upstream: TaskUpstreamSummary } | undefined> {
    ...
    const resolvedResourceUrl = await resolveResourceUrl(result.task)
    return {
      task: { ...result.task, resourceUrl: resolvedResourceUrl },
  ...
  async function listTasks(userId: number, options: {
    ...
    return {
      tasks: await Promise.all(taskList.map(async task => {
        const upstream = upstreamMap.get(task.upstreamId)
        const aimodel = aimodelMap.get(task.aimodelId)
        const resolvedResourceUrl = await resolveResourceUrl(task)
        return {
          ...task,
  ...
  async function submitTask(taskId: number): Promise<void> {
    ...
    const { task, upstream, aimodel, userApiKey } = data
    ...
    try {
      ...
      const service = await provider.createService(aimodel, userApiKey || undefined)
  ...
  async function handleSyncResult(task: Task, aimodel: Aimodel, result: { success: boolean; resourceUrl?: string; imageBase64?: string; mimeType?: string; error?: string }): Promise<void> {
    ...
    let resourceStorage: 'local' | 'cos' = 'local'
    ...
    if (result.resourceUrl) {
      ...
      if (response.ok) {
        ...
        const mimeType = result.mimeType || response.headers.get('content-type') || 'image/png'
        const ext = mimeType.split('/')[1]?.split(';')[0] || 'png'
        const fileName = `${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 10)}.${ext}`
        
        const saveResult = await saveFileUnified(buffer, fileName, mimeType)
        if (saveResult) {
          resourceUrl = saveResult.url
          resourceStorage = saveResult.storage
        }
      }
  ...
  async function syncTaskStatus(taskId: number): Promise<Task | undefined> {
    ...
    const service = await provider.createService(aimodel, userApiKey || undefined) as AsyncService
    const logPrefix = `[Task] #${task.id}`
    ...
    try {
      ...
      // 处理资源 URL：成功时下载并保存（根据配置选择本地或 COS）
      let resourceUrl = result.resourceUrl || null
      let resourceStorage: 'local' | 'cos' = 'local'
      let duration: number | undefined
      if (status === 'success' && resourceUrl && !resourceUrl.startsWith('/api/files/') && !resourceUrl.startsWith('/api/images/') && !resourceUrl.includes('.myqcloud.com')) {
        ...
        try {
          ...
          console.error(`${logPrefix} 保存资源失败:`, e)
        }
  ...
  async function executeAction(parentTaskId: number, customId: string, userId: number): Promise<Task> {
    ...
    const { task: parentTask, upstream, aimodel, userApiKey } = data
    ...
    const service = await provider.createService(aimodel, userApiKey || undefined) as MJService

 ...
 * 使用 /api/usage/token/ 接口（支持 API Key 直接认证）
 * @param baseUrl API 基础地址
 * @param apiKey API Key
 * @param userId 用户 ID（可选，暂不使用）
 * @param userIdHeader 用户 ID Header 名称（可选，暂不使用）
 */
async function queryOneApiBalance(
  ...
  // /api/usage/token/ 返回格式: { code: true, data: { name, total_available, total_used, unlimited_quota, ... } }
  if (data.code && data.data) {
    const d = data.data
    const quota = d.total_available || 0
    const usedQuota = d.total_used || 0
    const unlimitedQuota = d.unlimited_quota || false

    const upstreamInfo: UpstreamInfo = {
      username: d.name || '用户',
      displayName: d.name || '用户',
      quota,
      usedQuota,
      unlimitedQuota,

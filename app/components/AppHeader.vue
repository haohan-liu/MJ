...
// 导航项
const navItems: Array<{ label: string; to: string; icon: string; adminOnly?: boolean; external?: boolean }> = [
  { label: '创作', to: '/studio', icon: 'i-heroicons-paint-brush' },
  { label: '对话', to: '/chat', icon: 'i-heroicons-chat-bubble-left-right' },
  { label: '设置', to: '/settings', icon: 'i-heroicons-cog-6-tooth', adminOnly: true },
]

// 过滤导航项（管理员可见所有，普通用户隐藏管理员专属）
const visibleNavItems = computed(() => {
  return navItems.filter(item => !item.adminOnly || user.value?.role === 'admin')
})
  ...
  <header class="border-b border-(--ui-border) bg-(--ui-bg-elevated) flex-shrink-0">
    <div class="px-4 h-14 flex items-center justify-between">
      ...
      <NuxtLink to="/" class="flex items-center gap-2">
        <img v-if="siteLogo" :src="siteLogo" :alt="siteName" class="w-7 h-7" />
        <img v-else src="/logo.png" :alt="siteName" class="w-7 h-7" />

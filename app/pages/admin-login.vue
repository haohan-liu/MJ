...
async function handleAdminLogin() {
  if (!adminForm.username || !adminForm.password) {
    toast.add({ title: '请输入账号和密码', color: 'error' })
    return
  }

  isLoading.value = true
  try {
    const result = await $fetch<{ token: string; user: { id: number; username?: string; email?: string; name: string | null; role: string } }>('/api/auth/login', {
      method: 'POST',
    ...
    // 提取错误信息并显示 toast
    const errorMessage = error?.data?.message || error?.message || '账号或密码错误'
  ...
  <div class="min-h-screen bg-(--ui-bg) flex items-center justify-center p-6">
      <div class="w-full max-w-sm">
        ...
        <div class="text-center mb-8">
          ...
          <p class="text-(--ui-text-muted)">管理后台</p>
        </div>
        ...
        <div class="bg-(--ui-bg-elevated) backdrop-blur-sm rounded-lg p-8 border border-(--ui-border) shadow-2xl">
          ...
          <form @submit.prevent="handleAdminLogin" class="space-y-5">
            <!-- 账号 -->
            <div>
              <label class="block text-(--ui-text-toned) text-sm mb-2 font-medium">账号</label>
              <input
          ...
          <p class="mt-4 text-center text-sm">
            <NuxtLink to="/login" class="text-(--ui-primary) hover:opacity-80">
              普通用户登录
            </NuxtLink>
          </p>
        </div>

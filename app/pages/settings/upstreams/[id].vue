...
function validate(state: typeof form): FormError[] {
  ...
  // 开启用户余额显示时，必须选择平台类型
  if (state.showUserBalance && !state.upstreamPlatform) {
    errors.push({ name: 'upstreamPlatform', message: '开启用户余额显示时，必须选择平台类型' })
  }
  return errors
}
  ...
  if (!isNew.value && upstreamId.value) {
    ...
    if (upstream) {
      Object.assign(form, {
        ...
        showUserBalance: upstream.showUserBalance || false,
      ...
      if (upstream.aimodels) {
        aimodels.value = upstream.aimodels.map(m => ({
          ...
          uiCapabilities: m.uiCapabilities || null,
  ...
  try {
    if (isNew.value) {
      await createUpstream({
        ...
        showUserBalance: form.showUserBalance,
      ...
      <UForm id="upstream-form" :state="form" :validate="validate" class="space-y-6" autocomplete="off" @submit="onSubmit">
        ...
        <div class="max-w-2xl bg-(--ui-bg-elevated) rounded-lg p-6 border border-(--ui-border) space-y-4">
          ...
          <label class="flex items-center justify-between cursor-pointer">
            <div>
              <span class="text-(--ui-text)">显示用户余额</span>
              <p class="text-xs text-(--ui-text-muted) mt-1">开启后，用户可查看自己在该平台的余额（需在用户设置中配置 API Key）</p>
            </div>
            <UCheckbox v-model="form.showUserBalance" />
          </label>

          <UFormField v-if="form.showUserBalance" label="平台类型" name="upstreamPlatform">
            <USelect
              v-model="form.upstreamPlatform"
              :items="upstreamPlatformOptions.filter(o => o.value)"
              class="w-40"
              placeholder="选择类型"
              required
            />
            <template #hint>
              <span class="text-xs text-(--ui-text-muted)">选择上游平台类型，用于调用正确的余额查询接口</span>
            </template>
          </UFormField>
